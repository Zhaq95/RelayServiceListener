...
namespace WindowsServiceSap.Services
{
    public class OdbcSqlQueryExecutor
    {
        ...
        public async Task<List<Dictionary<string, object>>> ExecuteSqlQuery(QueryRequest queryRequest)
        {
            var results = new List<Dictionary<string, object>>();

            try
            {
                var connectionDetails = await _service.LoadConnectionDetails(queryRequest.ConnectorId);
                //var odbcConnectionString = $"Driver={{ODBC Driver 17 for SQL Server}};Server={connectionDetails.ServerAddress},Database={queryRequest.DatabaseName};{connectionDetails.PortNumber};UID={connectionDetails.UserName};PWD={connectionDetails.Password};";
                var odbcConnectionString = $"Driver={{ODBC Driver 17 for SQL Server}};" +
                           $"Server={connectionDetails.ServerAddress},{connectionDetails.PortNumber};" +
                           $"Database={queryRequest.DatabaseName};" + 
                           $"UID={connectionDetails.UserName};" +
                           $"PWD={connectionDetails.Password};";
                using (var connection = new OdbcConnection(odbcConnectionString))
                {
                    await connection.OpenAsync();

                    using (var command = new OdbcCommand(queryRequest.Query, connection))
