...
namespace WindowsServiceSap
{
    public sealed class RelayService : IDisposable
    {
        ...
        private OdbcSqlQueryExecutor _sqlQueryExecutor;
        ...
        public RelayService()
        {
            ...
            _sqlQueryExecutor = new OdbcSqlQueryExecutor();
        }
            ...
            try
            {
                ...
                using (var writer = new StreamWriter(context.Response.OutputStream, Encoding.UTF8, 1024, leaveOpen: true))
                {
                    ...
                    else if (requestName == "CheckConnectorStatus")
                    {
                        await HandleRequestAsync<QueryRequest>(context, writer, requestBody, async (req) =>
                        {
                            var result = await _connectorStatus.CheckStatus(req.Query,req.ConnectorId);
                            return JsonSerializer.Serialize(result); // Serialize the list to JSON
                        });
                    }
                    ...
                    if (requestObj == null)
                    {
                        await SendErrorResponseAsync(context, writer, HttpStatusCode.BadRequest, "Invalid request body.");
                        return;
                    }

                    // Find the connection matching the ConnectorId
                    var connectionDetails = allConnections.FirstOrDefault(c => c.ConnectorId == requestObj.ConnectorId);
                    if (connectionDetails == null)
                    {
                        await SendErrorResponseAsync(context, writer, HttpStatusCode.BadRequest, $"No connection found for ConnectorId: {requestObj.ConnectorId}");
                        return;
                    }

                    if (string.IsNullOrEmpty(connectionDetails.ConnectorType))
                    {
                        await SendErrorResponseAsync(context, writer, HttpStatusCode.BadRequest, "ConnectorType is missing or invalid.");
                        return;
                    }
                    ...
                    switch (connectionDetails.ConnectorType.ToLower().Replace(" ", ""))
                    {
                        case "saphana":
                            if (requestName == "ExecuteQuery")
                            {
                                await HandleRequestAsync<QueryRequest>(context, writer, requestBody, async (req) =>
                                {
                                    var result = await _queryExecutor.ExecuteSapQuery(req.Query,req.ConnectorId);
                            ...
                            else if (requestName == "ExecuteOdbcQuery")
                            {
                                await HandleRequestAsync<QueryRequest>(context, writer, requestBody, async (req) =>
                                {
                                    var result = await _queryExecutor.ExecuteSapOdbcQuery(req.Query,req.ConnectorId);
                                    return JsonSerializer.Serialize(result);
                                });
                            }
                        ...
                        case "odbcmssql":
                            if (requestName == "ExecutesqlQuery")
                            {
                                await HandleRequestAsync<QueryRequest>(context, writer, requestBody, async (req) =>
                                {
                                    var result = await _sqlQueryExecutor.ExecuteSqlQuery(req);
                                    return JsonSerializer.Serialize(result);
                                });
                            }
