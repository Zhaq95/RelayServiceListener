    ...
    public class ConnectionDetails
    {
        ...
        public async Task SaveOrUpdateConnectionDetailsAsync(ConnectionDetailsRequest connectionDetails)
        {
                ...
                var connectionData = new Dictionary<string, object>
            {
                ...
                { "ConnectorId", connectionDetails.ConnectorId },
            };
                ...
                string key = $"Connection_{connectionDetails.ConnectorId}";

                // Add or Update
                existingData[key] = connectionData;
        ...
        public async Task<ConnectionDetailsRequest> LoadConnectionDetailsAsync()
        {
            try
            {
                ...
                int connectorId = connectionData.GetProperty("ConnectorId").GetInt32();
        ...
        public async Task<ConnectionDetailsRequest> LoadConnectionDetails(int connectorId)
        {
            try
            {
                string filePath = Path.Combine(AppContext.BaseDirectory, "connectionDetails.json");
                ...
                var allConnections = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(json);

                if (allConnections == null || allConnections.Count == 0)
                    throw new Exception("No connections found in the JSON file.");

                foreach (var entry in allConnections)
                {
                    var conn = entry.Value;

                    int currentConnectorId = conn.GetProperty("ConnectorId").GetInt32();
                    if (currentConnectorId == connectorId)
                    {
                        return new ConnectionDetailsRequest
                        {
                            ServerAddress = conn.GetProperty("ServerAddress").GetString(),
                            PortNumber = conn.GetProperty("PortNumber").GetString(),
                            ConnectorType = conn.GetProperty("ConnectorType").GetString(),
                            UserName = DecryptField(conn.GetProperty("UserName").GetString()),
                            Password = DecryptField(conn.GetProperty("Password").GetString()),
                            ConnectorId = connectorId
                        };
                    }
                }

                throw new Exception($"No connection found for ConnectorId: {connectorId}");
            }
            catch (InvalidOperationException ex) { throw new InvalidOperationException(ex.Message); }
            catch (FileNotFoundException exi) { throw new FileNotFoundException("ConnectionDetails File not Found", exi); }
            catch (Exception ex)
        ...
        //public async Task<List<ConnectionDetailsRequest>> LoadAllConnectionDetailsAsync()
        //{
        //    try
        //    {
        //        string filePath = Path.Combine(AppContext.BaseDirectory, "connectionDetails.json");
        ...
        //        if (!File.Exists(filePath))
        //        {
        //            await logger.WriteLogAsync("ConnectionDetails.json file not found.");
        //            return new List<ConnectionDetailsRequest>();
        //        }
        ...
        //        if (data == null)
        //        {
        //            await logger.WriteLogAsync("Deserialized connection data is null.");
        //            return new List<ConnectionDetailsRequest>();
        //        }

        //        var result = new List<ConnectionDetailsRequest>();

        //        foreach (var entry in data)
        //        {
        //            var entryData = entry.Value;

        //            var conn = new ConnectionDetailsRequest
        //            {
        //                ServerAddress = entryData["ServerAddress"]?.ToString(),
        //                PortNumber = entryData["PortNumber"]?.ToString(),
        //                ConnectorType = entryData["ConnectorType"]?.ToString(),
        //                ConnectorId = Convert.ToInt32(entryData["ConnectorId"]),
        //                UserName = DecryptField(entryData["UserName"]?.ToString()),
        //                Password = DecryptField(entryData["Password"]?.ToString())
        //            };
        ...
        //    catch (Exception ex)
        //    {
        //        await logger.WriteLogAsync($"Error loading all connection details: {ex}");
        //        throw new Exception("An error occurred while loading all connection details.", ex);
        //    }
        //}
        ...
        public async Task<List<ConnectionDetailsRequest>> LoadAllConnectionDetailsAsync()
        {
            try
            {
                ...
                // Deserialize properly to Dictionary<string, JsonElement>
                ...
                foreach (var entry in data)
                {
                    var connData = entry.Value; // This is a JsonElement

                    var conn = new ConnectionDetailsRequest
                    {
                        ServerAddress = connData.GetProperty("ServerAddress").GetString(),
                        PortNumber = connData.GetProperty("PortNumber").GetString(),
                        ConnectorType = connData.GetProperty("ConnectorType").GetString(),
                        ConnectorId = connData.GetProperty("ConnectorId").GetInt32(),
                        UserName = DecryptField(connData.GetProperty("UserName").GetString()),
                        Password = DecryptField(connData.GetProperty("Password").GetString())
                    };
